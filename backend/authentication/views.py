...
from .serializers import FriendshipSerializer, RegistrationSerializer, LoginSerializer, CustomUserSerializer, UserUpdateSerializer
...
def get_friendship_amis(request):
    user = request.user
    amis_from = Friendship.objects.filter(
        to_user=user, status='accepted').values_list('from_user__username', flat=True)
    amis_to = Friendship.objects.filter(
        from_user=user, status='accepted').values_list('to_user__username', flat=True)
...
def accept_friendship_invitation(request, invitation_id):
    invitation = get_object_or_404(
        Friendship, pk=invitation_id, to_user=request.user, status='pending')
...
def send_friend_request(request):
    ...
    new_request = Friendship.objects.create(
